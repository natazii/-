...
const Game = {
  ...
  schedule(callback, delay) {
    ...
    const timer = setTimeout(() => {
      ...
      if (this.state === session && session && session.phase !== 'over') callback();
    }, delay);
  ...
  init(players, settings = {}) {
    this.timers.forEach(timer => clearTimeout(timer));
    this.timers.clear();
    settings = { startCash: 1500, startBonus: 200, jailFee: 50, evenBuilding: true, ...settings };
    const roster = settings.randomizeOrder ? shuffle(players) : players.slice();
    this.state = {
      ...
      turn: 0,
      phase: 'roll',        // roll | action | jail | auction | debt | over
      dice: [1, 1],
      doubles: 0,
  ...
  beginTurn() {
    ...
    if (p.skipTurns > 0) {
      ...
      GameUI.renderAll();
      return this.schedule(() => this.nextTurn(), 1200);
    }
    ...
    if (p.isBot) this.schedule(() => Bot.play(), 1100);
  },
  ...
  endTurn() {
    if (this.busy || !this.state || ['over', 'auction'].includes(this.state.phase)) return;
    ...
    if (this.state.pendingBuy !== undefined) {
      const idx = this.state.pendingBuy;
      this.state.pendingBuy = undefined;
      this.state.pendingBuyPlayer = undefined;
      if (this.state.settings.auction && this.state.owners[idx] === undefined) {
        return this.startAuction(idx);
      }
    }
    ...
    if (this.state.dice[0] === this.state.dice[1] && this.state.phase === 'action' && this.me.jail === 0) {
      ...
      GameUI.renderAll();
      this.schedule(() => this.roll(), this.me.isBot ? 420 : 260);
      return;
    }
    this.nextTurn();
  },
  ...
  async resolveTile(p) {
    ...
    switch (t.type) {
      ...
      case 'company': {
        ...
        if (owner === undefined) {
          if (p.cash >= t.cost || s.settings.auction) {
            s.pendingBuy = idx;
          ...
          if (s.settings.noRentInPrison && other.jail > 0) {
            this.log(`paid no rent on ${t.name}: the owner is in prison.`, p.id, 'info');
            break; // No rent means no Death Valley penalty, either.
          }
  ...
  rentOf(idx) {
    ...
    if (s.settings.noRentInPrison && this.player(owner).jail > 0) return 0;
  ...
  buy() {
    const s = this.state;
    if (!s || ['auction', 'debt', 'over'].includes(s.phase)) return;
  ...
  async jailRoll() {
    ...
    if (p.jail > 3) {
      ...
      this.state.vacationPot += this.state.settings.jailFee;
    ...
    this.state.phase = 'action';
    GameUI.renderAll();
    if (p.isBot) this.schedule(() => this.endTurn(), 900);
  },
  ...
  jailPay() {
    ...
    GameUI.renderAll();
    if (p.isBot) this.schedule(() => Bot.play(), 700);
  },
  ...
  enterDebt(p, creditor) {
    ...
    GameUI.renderAll();
    if (p.isBot) this.schedule(() => Bot.resolveDebt(), 350);
  },
  ...
  checkDebtResolved() {
    ...
    GameUI.renderAll();
    if (p.isBot) this.schedule(() => Bot.afterRoll(), 350);
    return true;
  },
  ...
  declareBankrupt() {
    if (!this.state || this.busy || ['over', 'auction'].includes(this.state.phase)) return;
    const p = this.me;
  ...
  /** v25.1 — resolve a country by its REAL name (tile.country), not by the
      board's colour group. Kuwait = Kuwait City + Hawalli only; Iraq =
      Baghdad + Mosul + Basra only; Bahrain and Qatar are their own lands. */
  countryIdsByKey(key) {
    const n = this.normalizeName(key);
    if (!n) return null;
    const countries = [...new Set(Object.values(COUNTRY_NAME))];
    let country = countries.find(c => this.normalizeName(c) === n);
    if (!country) {
      // a city name stands for its own country (hawalli → Kuwait, basra → Iraq)
      const tile = BOARD.find(t => t.type === 'city' && this.normalizeName(t.name) === n);
      if (tile) country = tile.country;
    }
    if (!country) {
      // demonyms / short forms (kuwaiti, uae, saudi …)
      const DEMONYMS = {
        kuwaiti: 'Kuwait', bahraini: 'Bahrain', qatari: 'Qatar', iraqi: 'Iraq',
        saudi: 'Saudi Arabia', egyptian: 'Egypt', tunisian: 'Tunisia',
        moroccan: 'Morocco', palestinian: 'Palestine', jordanian: 'Jordan',
        syrian: 'Syria', emirates: 'United Arab Emirates', uae: 'United Arab Emirates'
      };
      country = DEMONYMS[n] || null;
    }
    if (!country) return null;
    const ids = BOARD.map((t, i) => (t.type === 'city' && t.country === country) ? i : -1).filter(i => i >= 0);
  ...
  executeCommand(command) {
    ...
    if (this.state.phase === 'auction') return { ok: false, message: 'Finish the auction before using game commands.' };
    ...
    if (m) {
      ...
      const p = this.findPlayerByName(parsed.targetName);
      if (!p) return { ok: false, message: 'Player not found.' };
      const found = this.countryIdsByKey(parsed.rest);
      if (!found) return { ok: false, message: 'Country not found.' };
      const { country, ids } = found;
      if (action === 'give') {
        ids.forEach(i => this.giveLandTo(p, i));
        this.log(`received all land in ${country}`, p.id, 'buy');
        GameUI.renderAll();
        return { ok: true, message: `Gave ${country} (${ids.length} lands) to ${p.name}.` };
      }
      const owned = ids.filter(i => this.state.owners[i] === p.id);
      owned.forEach(i => this.takeLandFrom(p, i));
      this.log(`lost all owned land in ${country} by admin command`, p.id, 'build');
      GameUI.renderAll();
      return { ok: true, message: `Took ${owned.length} ${country} land${owned.length === 1 ? '' : 's'} from ${p.name}.` };
    }
  ...
  finishAction() {
    if (!this.state || ['over', 'debt', 'auction'].includes(this.state.phase) || this.me.cash < 0) return;
    this.state.phase = 'action';
    GameUI.renderAll();
    if (this.me.isBot) this.schedule(() => Bot.afterRoll(), 800);
  }
};
...
const Bot = {
  play() {
    const s = Game.state;
    if (!s || s.phase === 'over' || s.phase === 'auction') return;
    const p = Game.me;
    if (!p.isBot) return;
  ...
  afterRoll() {
    ...
    if (s.pendingBuy !== undefined && (s.pendingBuyPlayer === undefined || s.pendingBuyPlayer === p.id)) {
      ...
      // endTurn starts the auction before the bot can build or roll again.
      if (s.settings.auction) return Game.endTurn();
      s.pendingBuy = undefined;
      s.pendingBuyPlayer = undefined;
    }
    ...
    if (buildable.length) {
      ...
      return Game.schedule(() => Bot.afterRoll(), 500);
    }

    Game.schedule(() => Game.endTurn(), 700);
  }
};
