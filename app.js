// System State
    ...
    function startBackgroundMusic() {
      ...
      const unlock = function (e) {
        if (isMuted) return;
        if (e && e.target && e.target.closest && e.target.closest('#muteButton, #lobbyMuteButton')) {
          return;
        }
        tryPlayMusic();
        if (isMusicPlaying()) {
    ...
    function buildBoard() {
      ...
      tiles.forEach((tile) => {
        ...
        if (tile.icon) html += `<div class="rt-icon">${tile.icon}</div>`;
        if (tile.flag) html += `<div class="rt-flag">${tile.flag}</div>`;
        html += `<div class="rt-name">${tile.name}</div>`;
        div.innerHTML = html;
